"""
ConfigPoller Isolation Integration Test for MediaMonitor
Tests that ConfigPoller failures do not affect media update flow
...
# Test configuration
TEST_DURATION_MINUTES = 5  # 5 minutes test duration
CONFIG_FAILURE_DURATION_SECONDS = 60  # Simulate config failures for 60 seconds
...
class MockPythonServer:
    ...
    def __init__(self, port: int):
        ...
        self.server_log_file = "test_configpoller_server_log.txt"
        self.config_log_file = "test_configpoller_config_log.txt"
...
"""
            ...
            # Write server script
            with open('test_mock_configpoller_server.py', 'w', encoding='utf-8') as f:
            ...
            self.process = subprocess.Popen(
                [sys.executable, 'test_mock_configpoller_server.py'],
...
class ConfigPollerIsolationTest:
    ...
    def run_test(self) -> bool:
        """Run the ConfigPoller isolation test"""
        print("=" * 80)
        print("  MediaMonitor ConfigPoller Isolation Integration Test")
        print("=" * 80)
        ...
        print("\n📝 Test Plan:")
        print("   1. Run normally for 30 seconds (baseline)")
        ...
        print("=" * 80)
        print("  Phase 1: Baseline Operation (30 seconds)")
        print("=" * 80)
        baseline_end = datetime.now() + timedelta(seconds=30)
        ...
        try:
            ...
            print("=" * 80)
            print("  Phase 3: Config Recovery (30 seconds)")
            print("=" * 80)
            ...
            recovery_end = recovery_start + timedelta(seconds=30)
            ...
            if not config_recovered:
                print("⚠️ Config did not recover within 30 seconds")
    ...
    def analyze_results(self) -> bool:
        ...
        if self.failure_start_time and self.failure_end_time and len(self.updates) > 0:
            ...
            # Check for gaps in updates during failure period
            gaps_during_failure = []
            for i in range(1, len(updates_during_failure)):
                gap = (updates_during_failure[i].timestamp - updates_during_failure[i-1].timestamp).total_seconds()
                if gap > 10:  # Gaps > 10 seconds are significant
                    gaps_during_failure.append(gap)
            
            if len(gaps_during_failure) == 0:
                print(f"   ✅ No significant gaps in updates during failures")
            else:
                print(f"   ⚠️ {len(gaps_during_failure)} gaps > 10s detected during failures")
                print(f"   Maximum gap: {max(gaps_during_failure):.1f}s")
        else:
        ...
        # Check 1: Media updates continued during config failures
        if self.failure_start_time and self.failure_end_time:
            updates_during_failure = [u for u in self.updates 
                                     if self.failure_start_time <= u.timestamp <= self.failure_end_time]
            ...
            if len(updates_during_failure) == 0:
                passed = False
                issues.append("No media updates received during config failures")
        ...
        # Check 3: No significant gaps in updates during failures
        if self.failure_start_time and self.failure_end_time and len(self.updates) > 0:
            updates_during_failure = [u for u in self.updates 
                                     if self.failure_start_time <= u.timestamp <= self.failure_end_time]
            
            if len(updates_during_failure) >= 2:
                max_gap = 0
                for i in range(1, len(updates_during_failure)):
                    gap = (updates_during_failure[i].timestamp - updates_during_failure[i-1].timestamp).total_seconds()
                    max_gap = max(max_gap, gap)
                
                if max_gap > 30:  # Allow up to 30 seconds gap
                    passed = False
                    issues.append(f"Significant gap in updates during failures: {max_gap:.1f}s")
        ...
        if passed:
            ...
            print("  ✅ Media updates continued unaffected during config failures")
    ...
    def save_detailed_report(self):
        ...
        try:
            report_filename = f"configpoller_isolation_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
            
            with open(report_filename, 'w', encoding='utf-8') as f:
                f.write("MediaMonitor ConfigPoller Isolation Integration Test Report\n")
                f.write("=" * 80 + "\n\n")
                
                f.write(f"Test Configuration:\n")
                f.write(f"  Test duration: {TEST_DURATION_MINUTES} minutes\n")
                f.write(f"  Config failure duration: {CONFIG_FAILURE_DURATION_SECONDS} seconds\n\n")
                ...
                f.write(f"Config Requests ({len(self.config_requests)}):\n")
                for request in self.config_requests:
                    f.write(f"  {request}\n")
                f.write("\n")
...
def main():
    ...
    if not os.path.exists("MediaMonitor.csproj"):
        ...
        print("   cd MediaMonitor && python test_configpoller_isolation.py")
        return 1
